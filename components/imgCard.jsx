...
export default function ImgCard({ img }) {
  ...
  const { percentage , imgType } = useSettings();

  imgExportType = imgType
  ...
  useEffect(() => {
    ...
    a.href = canvas.toDataURL(`image/${imgExportType}`, 0.5);
  ...
  return (
    <figure id="img-item" className="w-full p-2 flex gap-3 border rounded-xl border-zinc-800 relative" >
      <div className="size-18 rounded-[6px] bg-black/20 absolute top-2 left-2"></div>
